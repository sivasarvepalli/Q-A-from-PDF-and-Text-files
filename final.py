...
def ui_qa_from_pdf():
    st.markdown("# Q&A from TEXT")
    st.write("Upload a PDF file and ask questions to get answers.")

    text = st.file_uploader("Upload a TEXT file")
...
def ui_qa_with_gpt():
    ...
    def ui_pdf_file():
        ...
        with t2:
            ...
            st.selectbox('select file', filenames, on_change=on_change, key='selected_file',
...
def main():
    ...
    selected_option = st.sidebar.radio(
        "Choose an option",
        ("Q&A from TEXT", "Q&A with PDF")
    )

    if selected_option == "Q&A from TEXT":
        ui_qa_from_pdf()
    elif selected_option == "Q&A with PDF":
        ui_qa_with_gpt()
